%% trackingInNonLinearManipulator_4trackers_REPAIRED_MPC.m
% Thesis appendix code for nonlinear reduced-UR5 trajectory tracking.
% The script executes each planned reference trajectory using PID, PID with
% inverse-dynamics feedforward, midpoint-linearized LTI-LQR, and online LTV-MPC
% tracking controllers, then exports tracking results, tables, and figures.
% Computational statements from the supplied script are preserved; comments have
% been condensed for thesis presentation.
...
trajectoryNames  = {'opt','cubic','quintic','trap'};
trajectoryLabels = {'MPC planned optimal trajectory','Cubic baseline trajectory', ...
                    'Quintic baseline trajectory','Synchronized trapezoidal baseline trajectory'};
...
plotTrajectoryName = 'opt';   % used only for the compact comparison plots below
...
for itraj = 1:numel(trajectoryNames)
    ...
    assert(isfield(planner_data,trName), 'planner_data.%s is missing. Run mpc.txt first.', trName);
    assert(isfield(planner_data.(trName),'q_ref'),   'planner_data.%s.q_ref is missing.', trName);
    assert(isfield(planner_data.(trName),'qd_ref'),  'planner_data.%s.qd_ref is missing.', trName);
    assert(isfield(planner_data.(trName),'qdd_ref'), 'planner_data.%s.qdd_ref is missing.', trName);
    checkTrajectoryDimensions(planner_data.(trName).q_ref, planner_data.(trName).qd_ref, ...
        planner_data.(trName).qdd_ref, N+1, trName);
end
...
exec_results.common.description = ['All planner trajectories executed on nonlinear reduced UR5 using PID, ', ...
...
exec_results.common.animation_trajectory_mode_map.mode_2 = 'exec_results.trajectories.cubic';
exec_results.common.animation_trajectory_mode_map.mode_3 = 'exec_results.trajectories.quintic';
exec_results.common.animation_trajectory_mode_map.mode_4 = 'exec_results.trajectories.trap';
...
fprintf('\n====================================================================\n');
fprintf(' Nonlinear tracking execution for all planner trajectories\n');
fprintf('====================================================================\n');
...
for itraj = 1:numel(trajectoryNames)
    ...
    checkTrajectoryDimensions(q_ref, qd_ref, qdd_ref, N+1, trName);
    ...
    elseif isfield(planner_data.(trName),'delta_tau') && isequal(size(planner_data.(trName).delta_tau), [3 N])
        u_nom(:,1:N) = planner_data.(trName).delta_tau;
        u_nom(:,N+1) = planner_data.(trName).delta_tau(:,N);
    elseif isfield(planner_data.(trName),'delta_tau') && isequal(size(planner_data.(trName).delta_tau), [3 N+1])
        u_nom = planner_data.(trName).delta_tau;
    else
        for k = 1:N+1
            u_nom(:,k) = reducedInverseDynamicsFeedforward(robot, idx, q_fix_456, q_ref(:,k), qd_ref(:,k), qdd_ref(:,k));
        end
    end
    ...
    trOut.q_ref = q_ref;
    trOut.qd_ref = qd_ref;
    trOut.qdd_ref = qdd_ref;
    trOut.u_nom = u_nom;
    trOut.ref_endpoint = reducedEndpointSeries(robot, reducedBody, q_ref, q_fix_456);
    trOut.lqrDesign = lqrDesign;
    trOut.mpcTracker = mpcTracker;
    if isfield(planner_data.(trName),'endpoint_path')
        trOut.planner_endpoint_path = planner_data.(trName).endpoint_path;
    end

    fprintf('\nTrajectory %d/%d: %s (%s)\n', itraj, numel(trajectoryNames), trName, trLabel);
    fprintf('--------------------------------------------------------------------\n');

    for ictrl = 1:numel(controllerNames)
        cname = controllerNames{ictrl};
        ...
        simOut.controller_label = controllerLabels{ictrl};
        simOut.endpoint = reducedEndpointSeries(robot, reducedBody, simOut.q, q_fix_456);
        simOut.ref_endpoint = trOut.ref_endpoint;
        simOut.reference_source = sprintf('exec_results.trajectories.%s', trName);
    ...
    exec_results.trajectories.(trName) = trOut;
end
fprintf('====================================================================\n\n');
...
exec_results.pid      = exec_results.trajectories.opt.trackers.pid;
exec_results.pid_ff   = exec_results.trajectories.opt.trackers.pid_ff;
exec_results.lqr      = exec_results.trajectories.opt.trackers.lqr;
exec_results.ltv_mpc  = exec_results.trajectories.opt.trackers.ltv_mpc;

%% Save results
save(outputFile, 'exec_results');
fprintf('Saved nonlinear execution results to %s\n', outputFile);
fprintf('Animation selector should read: exec_results.trajectories.(trajectoryName).trackers.(controllerName)\n\n');

%% PUBLICATION-QUALITY TRACKING RESULTS, TABLES, AND PLOTS
if enableSummaryPlots
    trackingResultsRoot = fullfile(pwd, 'publication_tracking_results');
...
plotSameReferenceJointTracking(exec_results, focusTrajectoryName, controllerNames, controllerLabelsLatex, pub, figDir);
plotSameReferenceTrackingErrors(exec_results, focusTrajectoryName, controllerNames, controllerLabelsLatex, pub, figDir);
plotSameReferenceControlEffort(exec_results, focusTrajectoryName, controllerNames, controllerLabelsLatex, pub, figDir);
plotSameReferenceEndpointTracking(exec_results, focusTrajectoryName, controllerNames, controllerLabelsLatex, pub, figDir);
plotSameReferenceScalarComparisons(metrics, trajectoryNames, controllerNames, trajectoryLabelsLatex, controllerLabelsLatex, focusTrajectoryName, pub, figDir);
plotCostOptimalityTimeHistories(exec_results, focusTrajectoryName, controllerNames, controllerLabelsLatex, pub, figDir);
plotCorrectionAndEnergy(exec_results, focusTrajectoryName, controllerNames, controllerLabelsLatex, pub, figDir);
...
plotCostBreakdownBars(exec_results, focusTrajectoryName, controllerNames, controllerLabelsLatex, pub, figDir);
